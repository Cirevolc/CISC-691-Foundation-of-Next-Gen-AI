...
RESULTS_DIR = Path("./results2").resolve()
...
def predict_sentiment(text, model, tokenizer):
    ...
    logger.info(f"Predict: [{text:<100}] : \t Sentiment: {sentiment}")
    return sentiment
...
def step_04_predict_sentiment(model_type= "fine_tuned" ):
    ...
    test_sentences = [
        "I loved this movie!",  # Should be Positive
        "This was the worst experience ever.",  # Should be Negative
        "It was okay, but not great.",  # Neutral (may vary)
    ]
    ...
    for sentence in test_sentences:
        predict_sentiment(sentence, model, tokenizer)
